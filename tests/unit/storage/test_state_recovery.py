...
def test_reset_rejects_forged_paths_without_mutation(tmp_path: Path) -> None:
    paths = _build_home(tmp_path)
    forged = replace(paths, application_db=paths.config_file)
    lease = StateLease.acquire(paths.home, StateLeaseMode.EXCLUSIVE)
    before = _inventory(paths.home)
    try:
        with pytest.raises(StateResetError) as raised:
            reset_local_state(forged, lease)
    finally:
        lease.close()
...
def test_reset_requires_matching_exclusive_lease_without_mutation(
    ...
    try:
        with pytest.raises(StateResetError) as raised:
            reset_local_state(paths, shared)
    finally:
        shared.close()
