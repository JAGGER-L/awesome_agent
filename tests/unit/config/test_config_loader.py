...
@pytest.mark.parametrize(
    "relative_path",
    ["config.toml", "awesome-agent.yaml"],
)
def test_legacy_user_config_files_are_rejected(
    tmp_path: Path,
    relative_path: str,
) -> None:
    home = tmp_path / "home"
    home.mkdir()
    (home / relative_path).write_text("legacy = true\n", encoding="utf-8")
    ...
    assert raised.value.code == "legacy_config_source"


def test_legacy_workspace_root_config_is_rejected_only_after_trust(
    tmp_path: Path,
) -> None:
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    (workspace / "awesome-agent.yaml").write_text("legacy: true\n", encoding="utf-8")
    paths = AwesomePaths.from_home(tmp_path / "home")
