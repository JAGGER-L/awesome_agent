...
def test_builtin_tool_metadata_controls_change_scope(tmp_path: Path) -> None:
    ...
    assert store.latest(workspace.key) is None

    execute_change_set = scope.change_set_for_tool(
        tool_name="execute",
        owner="operation_1",
        turn_id=None,
    )
    assert execute_change_set is not None
    assert store.get(execute_change_set) is not None
