...
Tasks 57-91 built the startup, sandbox, first-run configuration,
...
model diagnostics, startup-contract cleanup, and product-path regression
coverage. A 2026-07-03 implementation audit found that several product-surface
exit conditions were broader than the actual code path: user message turns
still bypass the durable Worker/Graph path, embedded `awesome` still has
planned/fake local Run actions, local skills/MCP remain disconnected, memory and
uploads are not end-to-end, and provider profiles are DeepSeek-only despite an
OpenAI adapter. The remaining product gap is now semantic closure: local
`awesome`, HTTP API, and operator CLI must expose the same durable execution
truth rather than separate direct-provider, placeholder, and Worker-owned paths.

This phase keeps `awesome` as a full-screen TUI. It intentionally does not add
an inline terminal chat mode. User message input is the only product execution
creation entry and should enter the Leader AgentLoop by default through an
internal conversation Run. Simple questions are leader turns with no tool
calls; coding tasks are leader turns that may call tools; complex work may
create teammates, subagents, and verifier work under runtime policy. Surfaces
do not create explicit product Runs and do not execute graphs directly.
...
The product closure baseline now treats the local `awesome` TUI as trusted
local execution:

- Local `awesome` runs user message turns directly in the launch/current working
  directory. Reads, writes, and shell commands operate on the same real paths a
  user would touch from their terminal.
- Trusted local execution is not a security boundary. Guardrails reduce
  accidental damage and make risky actions visible; they do not claim to
  confine a malicious process.
- User-visible generated files belong in the launch/current working directory,
  with changed-file summaries and previews shown in the transcript. `artifact`
  projections index or preview real files and bounded run evidence; they are not
  a second user-facing output workspace.
- Host thread workspaces and git worktrees are not the default user-turn
  isolation mechanism. Git worktrees remain a repository-task concept only when
  an explicit future repository isolation task reintroduces that path.
- Docker/AIO execution is deferred technical debt for the current product
  closure sequence. The durable local product path should not depend on AIO
  Docker semantics.
...
| --- | --- | --- | --- | --- |
| Task 76 | Embedded Local Runtime Host | Partial | Make `awesome` default to an embedded local runtime host where user messages can create resumable durable conversation Runs without a running HTTP API. | Embedded user message turns now create durable conversation Runs and execute the local `conversation-turn` graph, but cancel, approval, budgets, artifacts, extension catalogs, and durable local runtime storage still need parity work. Task 95 owns that remaining closure. |
| Task 77 | Nonblocking Streaming And Resume | Partial | Make local and HTTP execution stream incrementally without freezing the TUI, and make interrupted user message turns resumable. | Nonblocking Textual workers and provider-stream rendering exist; Task 96 replaced the old stream-turn resume parameter surface with thread-first continuation. Richer routed streaming and SSE catch-up remain Task 97. |
| Task 78 | Thread Session And Continuation UX | Done | Make thread navigation feel like conversation management instead of backend state dumping. | First user message auto-creates a thread from launch context; `/new` and `/threads` switch/load conversations cleanly; duplicate titles are distinguishable; full UUIDs and container paths stay out of normal transcript output. |
| Task 79 | Transcript UI Redesign | Done | Make the full-screen TUI read like a coding-agent chat product rather than a raw log or operator dashboard. | User input, assistant output, command results, system notices, errors, run/tool events, and artifacts have distinct compact rendering; welcome chrome stays minimal; `/help` and long output never remove the input box. |
...
| Task 81 | Model Routing Diagnostics | Done | Make configured/requested/observed model routing explainable. | `/model` and turn completion metadata show configured model, provider, base URL, env key presence, requested model, response model, and response id where available; model self-description is not treated as authority; no price/cost fields appear. |
| Task 82 | Startup Contract Cleanup | Done | Align docs, CLI help, quickstart, and roadmap with product startup semantics. | `awesome` is documented as the local full-screen TUI entry; `make dev` is local API development; `make docker-start` is Docker API deployment; `awesome-agent start` is fallback/debug only; normal work does not require an explicit product Run command. |
| Task 83 | Leader AgentLoop Primary Turn Path | Done | Remove the user-message provider bypass and make every normal TUI/API conversation turn enter a durable Leader AgentLoop path. | Task 92 replaced the terminal `leader-turn` provider bypass with durable conversation Run intake, Worker routing, `ConversationGraph` execution, and runtime-event projection. |
| Task 84 | Transcript Scrollback And Per-Turn Thought | Done | Make the TUI transcript usable for long, multi-turn sessions. | Transcript auto-scrolls to the newest content, mouse/keyboard scrollback works, slash commands echo before results, thought blocks attach to the correct turn, and `Ctrl+O` toggles the current/recent thought without overwriting prior turns. |
| Task 85 | Slash Command Surface Cleanup | Done | Reduce slash commands to the product control surface and delete obsolete commands without compatibility shims. | `/resume`, `/models`, `/uploads`, `/artifacts`, and `/switch` are removed from parsing, help, autocomplete, docs, and tests; retained commands render concrete user-facing content rather than raw backend dictionaries. |
| Task 86 | Model, Thinking, Memory, And Skill Pickers | Done | Convert model, thinking, memory, and skill controls into interactive conversation/turn controls. | `/model` selects the conversation model; `/thinking` selects Off/On high/On max; `/memory` manages local and provider memory; `/skills` stages skills for the next turn only and clears them after use. |
| Task 87 | Tools, Approval, And Team Event Rendering | Done | Show leader tools, file/command approvals, tool calls, team activity, and verifier output in a compact collapsible form. | Leader-visible tools are grouped by user concepts; file edits and risky commands ask by default; session-only allow rules are visible; tool/team/subagent/verifier events are folded by default and expanded by `/details`. |
| Task 88 | Workspace Outputs And Changed Files | Done | Make user-visible generated files belong to the launch workspace and replace the `/artifacts` user surface with changed-file summaries. | Outputs are written to the active workspace/project, run audit artifacts remain internal evidence, task completion shows created/updated/deleted files, and `/threads` shows changed-file counts ordered by most recent conversation activity. |
| Task 89 | Local Durable Sessions And Threads | Done | Persist embedded local TUI conversations across process restarts. | Local `awesome` uses a durable local conversation store, `/threads` lists previous conversations after restart, thread selection restores transcript/model/thinking/memory metadata, and no HTTP API is required for local persistence. |
| Task 90 | CLI Product Polish E2E | Done | Verify the full-screen TUI as a usable local coding-agent product. | Headless and product-path E2E cover long transcript scrolling, command echo, model/thinking/memory/skill pickers, approvals, changed files, team event rendering, local restart recovery, and a simple HTML game generated through user input. |
| Task 91 | CLI Runtime Regression Fixes | Reopened | Correct the remaining product-path regressions where local `awesome` still behaved like direct provider chat for user message turns. | Local user message turns now use durable conversation Runs through Task 92. Remaining regressions are local skills/MCP/cancel/approval placeholders and runtime parity gaps tracked by Tasks 95-98. |

Planned product closure sequence:

| Task | Phase | Status | Purpose | Exit condition |
| --- | --- | --- | --- | --- |
| Task 92 | Durable Conversation Turn Authority | Done | Make user message turns a durable Leader Run/Worker/Graph path rooted in the user's trusted local current working directory, not a direct provider/tool loop. | Local/API user message turns create `conversation` Runs with Leader agents, `conversation-turn` routing, runtime events, user/assistant message writes owned by `ConversationGraph`, model/tool execution owned by the graph, usage and changed-file projection, and terminal Run state. `ConversationService` is now a runtime-event projection boundary. Checkpointing, full budget ledger parity, routed-provider streaming, and resume/cancel/approval remain in Tasks 95-97. |
| Task 93 | Trusted Local Guardrails | Done | Add Hermes-style soft guardrails for trusted local execution: defense-in-depth checks that prevent accidents and make risky actions visible without pretending to be a sandbox. | File, patch, and terminal execution now share guardrail policy; shell commands pass allow/ask/deny classification and regex approval gates; sensitive reads are refused; sensitive patch writes require approval; extreme destructive commands are hard-blocked; `AWESOME_AGENT_WRITE_SAFE_ROOT` optionally bounds patch writes; LocalSandbox subprocess environments are scrubbed of provider/API secret-looking names; tool results carry guardrail evidence; docs state trusted local mode is not a security boundary. Task 96 closes exact-invocation approval decisions without session or always grants. |
| Task 94 | Output Redaction | Done | Redact likely secrets before command output, logs, run evidence, or tool results enter model context or user-visible projections. | stdout/stderr, tool results, text artifacts, runtime events, validation summaries, application logs, API errors, SSE payloads, diagnostics, and transcript projections redact common API keys, auth headers, private keys, JWTs, database URLs, URL passwords, query tokens, and credential assignments while preserving enough context for debugging; raw unredacted tool output is not persisted as durable run evidence by default. |
| Task 95 | Real Embedded Local Runtime | Planned | Make embedded `awesome` use the same Worker-style runtime authority as API runs instead of a direct graph executor or planned-run shim. | User message turns create conversation Runs with Leader agents and execute only through the embedded Worker pump / `DurableWorker.run_once()` path backed by local SQLite runtime state. Direct graph execution, planned-run shim state, in-memory product runtime state, hard-coded local cancel/approval responses, explicit user/API/TUI/CLI Run creation entrances, and legacy transcript Run-message projection are removed. Local status, runs, usage, artifacts, and errors project from real runtime state. Embedded local/chat-first execution uses trusted `working_directory` and does not construct repository registry or managed worktrees. |
| Task 96 | Durable Turn Resume Cancel Approval | Done | Complete interrupted-turn, cancellation, and approval lifecycle semantics on top of the Worker-style local/API runtime path established by Task 95. | Thread-first continuation uses `POST /threads/{thread_id}/turns/continue/stream`, resumes only the newest recoverable conversation Run, does not create a new user message or Run, and emits `turn.continued`. Cancellation is request/terminal rather than fake pause, approval decisions are exact invocation only, persist/requeue the pending invocation, and top-level Run write endpoints plus run-first resume are removed. Routed streaming catch-up remains Task 97 and API resource completeness remains Task 105. |
| Task 97 | Runtime Streaming Event Projection | Done | Add streaming over routed Worker-owned runtime execution without weakening checkpoint or dispatch ownership. | Routed provider streaming now supports fallback before visible output, durable stream events carry top-level `run_id` and `runtime_sequence`, `continue/stream` supports runtime cursor catch-up, embedded local mode live-fans out Worker-owned runtime events, and HTTP/local/TUI clients consume explicit tool/team/validation/model-attempt projections from the same runtime-event contract. Docker/AIO validation remains deferred to Task 104. |
| Task 98 | Local Capability Surface Parity | Planned | Wire local skills, MCP, community tools, and extension catalogs into the embedded Worker-style trusted-cwd runtime with the same resolver/executor authority as API runs. | Local `list_skills`, `mcp_status`, `list_tools`, and staged skill selection read a real extension catalog; granted MCP/community tools execute through the Task 95 local runtime container, `ToolExecutor`, approval, timeouts, cancellation, durable invocation records, and `CapabilityResolver`; local tool execution inherits trusted cwd, guardrail, env-scrub, and redaction policy; denied tools are inspectable and cannot be exposed by prompt convention or bypassed by extension adapters. |
| Task 99 | Memory Lifecycle Integration | Planned | Connect builtin file memory and optional provider contracts to the actual conversation lifecycle instead of exposing settings-only or heuristic local facts. | Builtin memory stores structured entries in `settings.local_state_dir / "memory"` `USER.md` and `MEMORY.md`; enabled turns inject bounded untrusted memory context; model writes use only `memory.manage`; API/TUI surfaces expose real status/list/delete without add forms; provider failures are isolated; full source, raw conversations, secrets, and visible reasoning remain excluded. |
| Task 100 | Thread Upload And Attachment Lifecycle | Planned | Turn thread uploads from an empty inspection endpoint into a real product capability. | API supports validated upload/create/list/delete for thread attachments; TUI can attach files to a turn; attachments are persisted as thread metadata or copied into user-approved cwd paths with size/type limits, redaction policy, and model/tool visibility rules; attachment state survives restart and appears in diagnostics without exposing raw secrets. |
| Task 101 | DeepSeek Provider Profile Productization | Planned | Productize the current DeepSeek-only model selection path without adding another provider yet. | Settings, API `/models`, TUI picker, RoleModelResolver, ModelProviderFactory, readiness checks, routed streaming, usage, errors, and observed/requested metadata use one reusable provider-profile contract while only DeepSeek is supported. Provider credentials remain runtime/provider-owned and are not passed to subprocess or tool environments; unsupported providers fail with actionable errors; OpenAI, Anthropic, Ollama, and other providers remain future work outside the committed product-closure sequence. |
| Task 102 | Cwd Instruction Context | Planned | Make CWD instruction files a deterministic runtime context source for conversation turns without reading API server CWD. | `AGENTS.md` and `CLAUDE.md` are discovered only as direct children of the Run `working_directory`; `AGENTS.md` has precedence over `CLAUDE.md`; duplicate `CLAUDE.md` lines are removed; oversized files are skipped; context is injected as a runtime/system block for every model call in the turn; snapshot metadata and durable evidence are recorded without storing full file contents; README, repository metadata, UI/API status surfaces, and tool-layer behavior are out of scope. |
| Task 103 | TUI Changed Files Summary Polish | Planned | Make changed-file evidence readable and recoverable in the chat transcript without adding file, diff, artifact, or internal evidence preview surfaces. | Assistant messages render structured `changed_files` metadata beneath the assistant response; the first three changed files are visible by default, remaining files are folded behind a count and can be expanded/collapsed with a keyboard shortcut for the current TUI session; reload restores summaries from assistant message metadata; the old extra system-message rendering path is removed; no file opening, diff preview, artifact preview, slash command, API endpoint, or internal evidence UI is added. |
| Task 104 | TUI Status Panel | Done | Provide a Claude-style tabbed `/status` panel for the current conversation, project configuration, and usage without turning the TUI into an operator dashboard. | `/status` opens a fixed panel with `Status`, `Config`, and `Usage` tabs switched by left/right keys and closed by `Esc` or normal chat flow; the transcript keeps only the `/status` command echo and does not append status tab content as system messages. `Status` shows version placeholder, conversation name, persistent thread id, cwd, and model; `Config` shows project config/env, provider, API key state, base URL, memory summary, and sandbox; `Usage` shows cost placeholder, TUI-session duration, and input/output tokens. Compact status lines, dashboards, runtime/debug fields, readiness full checks, Docker/AIO checks, cache usage, cost calculation, version lookup, and API resource completeness are out of scope. |
| Task 105 | API Resource Completeness | Done | Fill API contract gaps after runtime authority is unified across API and embedded local surfaces, while preserving the Task 104 status-panel data contract. | Thread memory, attachments, artifacts, usage, events, runs, messages, approvals, skills, MCP, tools, diagnostics, config, model, and status-supporting resources have consistent request validation, pagination or bounded limits, response schemas, structured errors, and OpenAPI descriptions; thread-level projections include user message turns and internal conversation Runs derived from runtime evidence. API completeness owns schema parity for fields consumed by the Task 104 status panel, including project config/env metadata and provider base URL where available. The legacy `/threads/{thread_id}/uploads` placeholder is removed; file input is represented by thread attachments. API completeness does not carry embedded local fake-runtime cleanup, which is owned by Task 95. |
| Task 106 | Onboarding And Human Error Recovery | Done | Make first-run setup and common local CLI failures understandable to a new user. | `awesome init`, `awesome doctor`, the TUI welcome panel, `/config`, and model-error rendering now explain missing user config, missing API key, unsupported DeepSeek base URL, current cwd, and safe next steps without adding API/Docker/Worker readiness scope. |
| Task 107 | Run Lifecycle Convergence And Stuck-Turn Recovery | Done | Ensure local conversation Runs always converge to a terminal or recoverable state, so provider stalls, cancellation, or stale local Worker execution cannot permanently lock the TUI input. | Runtime-owned cancellation is two-phase and terminal; provider/model-call setup emits durable pre-call, start, completion, and failure evidence with a bounded first-event timeout; embedded local startup and `awesome doctor` reconcile stale `running` or `executing` Runs into `cancelled` or `recovery_required`; TUI active-operation state unlocks on terminal, failed, cancelled, or recovery-required projections; repeated Ctrl+C is idempotent and user-visible without killing the active runtime stream worker. Docker/API recovery breadth, provider ecosystem expansion, and new operator dashboards are out of scope. |
| Task 108 | Model I/O Isolation | Done | Make conversation turns converge even when the provider SDK stream blocks before first output, stalls mid-stream, or ignores cooperative cancellation. | ConversationGraph now consumes model events through `ModelExecutionService`; production local/API Worker paths use a killable subprocess backend with first-event, idle, total, and cancellation deadlines; timeout evidence is persisted as model-call phases and converted into terminal runtime failure instead of retrying a wedged stream; test paths can still inject in-process fake providers; TUI tool status labels render successful tools in green and failed tools in red. Persistent warm model worker pools remain technical debt. |
...
| Task 65 | CLI Context | Done | Made `awesome` usable from any user project directory by treating launch cwd as the default thread context. | The TUI starts with the launch repo/workspace context and user input no longer blocks on manual repository selection. |
...
Tasks 92-107 are the committed product-closure sequence that must make local
`awesome` and HTTP API surfaces share one durable execution truth. The
following phases remain directional beyond those tasks. They are not committed
task numbers until this roadmap is updated through change control.

| Phase | Direction | Entry criteria | Exit shape |
| --- | --- | --- | --- |
| Provider Ecosystem Phase | Expand fallback policy, provider-quality feedback, and provider breadth after the DeepSeek-only Task 101 contract is stable. | Tasks 97 and 101 complete; provider routing supports streaming, profiles, usage, errors, and readiness through one reusable provider contract while the committed product remains DeepSeek-only. | Future provider additions enter through the same contract with contract tests and product-surface metadata rather than one-off adapters. |
| Operations Phase | Improve dashboards, alerts, trace exploration, recovery metrics, and readiness diagnostics. | Tasks 92-98 and 105 complete; durable evidence from user message turns, internal conversation Runs, local embedded mode, and HTTP mode is queryable through one schema. | Operators can diagnose latency, failure class, budget pressure, provider quality, recovery behavior, extension health, sandbox health, and worker health without reading raw logs. |
| Web And Multi-Surface Phase | Add a Web frontend and richer multi-client workflows over the same conversation, Run, command, tool, artifact, upload, and memory contracts used by the full-screen TUI. | Tasks 92-107 complete; Conversation API, shared client stream events, command metadata, uploads, memory, and preview resources are stable. | Web, TUI, and API clients share backend contracts without duplicating AgentLoop, model chat, tool, upload, memory, or policy logic. |
| Team Intelligence Phase | Improve Leader planning, assignment quality, Verifier calibration, and recovery tuning. | Tasks 92-98 close the user-turn/runtime split and P5 evidence exists across real team runs and provider metrics. | Team behavior improves through bounded, observable policy changes rather than hidden prompt growth. |
...
- Do not start broad provider, MCP, skill, UI, or DeerFlow-style expansion while
  a kernel-phase or product-closure task remains open unless that work is the
  explicit purpose of the active task.
