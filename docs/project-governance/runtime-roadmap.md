...
| Task 95 | Real Embedded Local Runtime | Planned | Make embedded `awesome` use the same Worker-style runtime authority as API runs instead of a direct graph executor or planned-run shim. | User message turns create real conversation Runs and execute through a local runtime container composed of `RuntimeService`, `ConversationService`, local SQLite runtime repository, artifact store/repository, budget repository, dispatcher, and `DurableWorker.run_once()` or an equivalent embedded worker pump. The old direct graph executor, `_planned_runs`, in-memory runtime state, hard-coded local cancel/approval responses, TUI/API/CLI explicit Run creation entrances, and `ThreadMessageKind.RUN` transcript projection are deleted rather than kept as compatibility paths; local status, runs, usage, artifacts, and errors are projected from real runtime state. Embedded local/chat-first execution uses trusted `working_directory` and does not construct repository registry or managed worktrees. Task 95 does not delete legacy managed-worktree runtime internals, and does not guarantee full local/Postgres repository parity, cross-restart resume/cancel/approval recovery, local skills/MCP parity, or routed-provider streaming. |
