...
| Task 104 | TUI Execution Controls | Planned | Expose real execution mode and runtime health controls from the embedded Worker-style runtime without turning the TUI into an operator dashboard. | Users can see trusted local cwd, sandbox backend, guardrail/env-scrub status, active run/route/embedded worker pump or worker state, dispatch state, and pending approvals; user input remains the primary route and Docker/AIO controls are not required for the product closure path. |
