...
| Task | Phase | Status | Purpose | Exit condition |
| --- | --- | --- | --- | --- |
| Task 92 | Durable Conversation Turn Authority | Done | Make ordinary `/threads/{thread_id}/turns` a durable Leader Run/Worker/Graph path rooted in the user's trusted local current working directory, not a direct provider/tool loop. | Ordinary local/API turns create `conversation` Runs with Leader agents, `conversation-turn` routing, runtime events, user/assistant message writes owned by `ConversationGraph`, model/tool execution owned by the graph, usage and changed-file projection, and terminal Run state. `ConversationService` is now a runtime-event projection boundary. Checkpointing, full budget ledger parity, routed-provider streaming, resume/cancel/approval, and guardrails remain in Tasks 93 and 95-97. |
| Task 93 | Trusted Local Guardrails | Planned | Add Hermes-style soft guardrails for trusted local execution: defense-in-depth checks that prevent accidents and make risky actions visible without pretending to be a sandbox. | File tools, patch tools, and terminal commands pass shared path/command guards; terminal commands pass a regex approval gate before execution; sensitive reads/writes require approval or are refused; extreme destructive commands are hard-blocked; `AWESOME_AGENT_WRITE_SAFE_ROOT` provides the optional hard write boundary; subprocess environments are scrubbed of provider/API secrets; once/session bypasses are explicit user decisions; guardrail decisions are visible in durable evidence and docs state this is not a security boundary. |
