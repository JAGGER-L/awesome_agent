...
| Task 77 | Nonblocking Streaming And Resume | Partial | Make local and HTTP execution stream incrementally without freezing the TUI, and make interrupted user message turns resumable. | Nonblocking Textual workers and provider-stream rendering exist; Task 96 replaced the old `resume_run_id` surface with thread-first continuation. Richer routed streaming and SSE catch-up remain Task 97. |
