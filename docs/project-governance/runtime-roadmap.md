...
| Task 95 | Real Embedded Local Runtime | Planned | Make embedded `awesome` use the same Worker-style runtime authority as API runs instead of a direct graph executor or planned-run shim. | User message turns create conversation Runs with Leader agents and execute only through the embedded Worker pump / `DurableWorker.run_once()` path backed by local SQLite runtime state. Direct graph execution, `_planned_runs`, in-memory product runtime state, hard-coded local cancel/approval responses, explicit user/API/TUI/CLI Run creation entrances, and `ThreadMessageKind.RUN` transcript projection are removed. Local status, runs, usage, artifacts, and errors project from real runtime state. Embedded local/chat-first execution uses trusted `working_directory` and does not construct repository registry or managed worktrees. |
