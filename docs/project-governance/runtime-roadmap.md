...
| Task 93 | Trusted Local Guardrails | Planned | Add the minimum defense-in-depth guardrails required for trusted local execution without pretending they are a sandbox. | File tools, patch tools, and terminal commands pass shared path/command guards; sensitive writes require approval or are refused, extreme destructive commands are hard-blocked, `AWESOME_AGENT_WRITE_SAFE_ROOT` provides an optional hard write boundary, subprocess environments are scrubbed of provider/API secrets, and guardrail decisions are visible in durable evidence. |
...
| Task 98 | Local Capability Surface Parity | Planned | Wire local skills, MCP, community tools, and extension catalogs into the embedded runtime with the same resolver/executor authority as API runs. | Local `list_skills`, `mcp_status`, `list_tools`, and staged skill selection read a real extension catalog; granted MCP/community tools execute through `ToolExecutor`, approval, timeouts, cancellation, durable invocation records, and `CapabilityResolver`; denied tools are inspectable and cannot be exposed by prompt convention. |
...
| Task 101 | Provider Profile Productization | Planned | Make model selection provider-aware instead of DeepSeek-only. | Settings, API `/models`, TUI picker, RoleModelResolver, ModelProviderFactory, and readiness checks support DeepSeek and OpenAI through the same profile schema; configured/requested/observed provider metadata is accurate; unsupported providers fail with actionable errors; Anthropic/Ollama remain future work until this provider contract is stable. |
| Task 102 | Repository Instruction Context | Planned | Make project instructions a deterministic context source for local cwd and repository-aware turns. | AGENTS.md, CLAUDE.md, README, and relevant project metadata are discovered once per turn through bounded local tools or context middleware, injected with provenance and budget limits, and reflected in durable evidence; missing or conflicting instructions produce visible uncertainty instead of silent prompt assumptions. |
