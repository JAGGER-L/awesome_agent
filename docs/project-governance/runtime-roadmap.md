...
| Task 77 | Nonblocking Streaming And Resume | Partial | Make local and HTTP execution stream incrementally without freezing the TUI, and make interrupted user message turns resumable. | Nonblocking Textual workers and provider-stream rendering exist, but durable local resume is not wired and HTTP `resume_run_id` is not propagated through the conversation API. Task 96 owns resume/cancel/approval semantic closure. |
...
| Task 96 | Durable Turn Resume Cancel Approval | Planned | Complete interrupted-turn, cancellation, and approval lifecycle semantics on top of the Worker-style local/API runtime path established by Task 95. | `resume_run_id` flows through TUI, surface clients, API schemas, and runtime services; `continue` / `resume` resumes the last resumable durable turn; cancellation interrupts active Worker/Graph work through dispatcher state instead of only stopping rendering; approval decisions requeue the exact pending invocation; transcript and run state recover after process restart. Task 96 does not introduce the embedded Worker path itself. |
