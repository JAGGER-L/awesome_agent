...
| Task 101 | Provider Profile Productization | Planned | Make model selection provider-aware instead of DeepSeek-only. | Settings, API `/models`, TUI picker, RoleModelResolver, ModelProviderFactory, and readiness checks support DeepSeek and OpenAI through the same profile schema; configured/requested/observed provider metadata is accurate; provider credentials remain runtime/provider-owned and are not passed to subprocess or tool environments; unsupported providers fail with actionable errors; Anthropic/Ollama remain future work until this provider contract is stable. |
| Task 102 | Cwd Instruction Context | Planned | Make current-working-directory instructions a deterministic context source for local turns, with repository metadata added only when the cwd is actually a repository. | AGENTS.md, CLAUDE.md, README, and relevant cwd metadata are discovered once per turn through bounded local tools or context middleware, injected with provenance and budget limits, and reflected in durable evidence; repository metadata is included only when applicable; missing or conflicting instructions produce visible uncertainty instead of silent prompt assumptions. |
| Task 103 | TUI Evidence Preview | Planned | Let users inspect what the agent changed or produced without leaving the TUI. | Changed-file summaries can open bounded diff, file, and artifact previews for real cwd files and internal run evidence; generated files are distinguishable from internal audit artifacts; previews are redacted, size-bounded, keyboard accessible, and recoverable after transcript reload. |
| Task 104 | TUI Execution Controls | Planned | Expose real execution mode and runtime health controls from the embedded Worker-style runtime without turning the TUI into an operator dashboard. | Users can see trusted local cwd, sandbox backend, guardrail/env-scrub status, active run/route/embedded worker pump or worker state, dispatch state, and pending approvals; user input remains the primary route. Local product readiness and tests must not invoke Docker CLI or AIO checks by default; Docker/AIO readiness belongs only behind an explicit Docker/API profile or deferred technical-debt path. |
| Task 105 | API Resource Completeness | Planned | Fill API contract gaps after runtime authority is unified across API and embedded local surfaces. | Thread memory, uploads, artifacts, usage, events, runs, messages, approvals, skills, MCP, tools, and diagnostics have consistent request validation, pagination or bounded limits, response schemas, structured errors, and OpenAPI descriptions; thread-level projections include user message turns and internal conversation Runs derived from runtime evidence. API completeness does not carry embedded local fake-runtime cleanup, which is owned by Task 95. |
| Task 106 | Onboarding And Human Error Recovery | Planned | Make first-run setup and common failures understandable to a new local user. | Welcome, `awesome init`, `/config`, `/model`, readiness, provider-key errors, trusted local cwd, and guardrails explain what is missing, why it matters, where to set it on Windows/macOS/Linux, how to verify it, and what safe first command to try. |
| Task 107 | Headless Chat CLI | Planned | Add a non-TUI chat path for scripts, CI, and terminal users while reusing the same conversation authority. | `awesome-agent chat` can create/resume a thread, stream a durable conversation turn, pass model/memory/skill options, attach files when Task 100 is available, and render structured errors without duplicating AgentLoop or TUI logic. |
| Task 108 | Local API Hardening | Planned | Make API exposure rules match the product deployment boundary. | Loopback remains unauthenticated by default; non-loopback binding requires explicit auth/rate-limit configuration or is refused; readiness and docs identify the active security mode; rate limiting protects long-running run creation, uploads, and streaming endpoints. |
| Task 109 | Provider Ecosystem Expansion | Planned | Add provider breadth only after the provider-profile contract is no longer DeepSeek/OpenAI-specific. | Anthropic/Claude and local-model providers enter through the same ModelProvider, routing, streaming, usage, error, readiness, and profile contracts; provider additions include contract tests and product-surface metadata rather than one-off adapters. |
| Task 110 | Conversation Export | Planned | Add a bounded conversation export path after durable execution, memory, upload, and provider semantics are stable. | Users can export conversation history with thread metadata, model/usage summaries, changed-file summaries, attachment references, and redacted run evidence without exposing raw secrets, full tool output, or unbounded prompt context. |
| Task 111 | Extension Config UI | Planned | Add a local UI for inspecting and editing MCP and extension configuration after execution authority and extension parity are stable. | Users can inspect and update MCP sources, project skill roots, and extension health through a local UI that writes the same file-based project configuration, preserves redaction, and never bypasses extension capability policy. |
...
Tasks 92-111 are the committed product-closure sequence that must make local
`awesome`, HTTP API, and CLI clients share one durable execution truth. The
...
| --- | --- | --- | --- |
| Provider Ecosystem Phase | Expand fallback policy, provider-quality feedback, and provider breadth beyond the Task 101/109 contracts. | Tasks 97, 101, and 109 complete; provider routing supports streaming, profiles, usage, errors, and at least one non-DeepSeek provider through the same contract. | Provider decisions are reliable, explainable, retry-safe, and tuned by measured runtime outcomes rather than hard-coded optimism. |
