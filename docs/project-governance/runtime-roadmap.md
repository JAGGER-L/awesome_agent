...
| Task 98 | Thread Upload And Attachment Lifecycle | Planned | Turn thread uploads from an empty inspection endpoint into a real product capability. | API supports validated upload/create/list/delete for thread attachments; TUI can attach files to a turn; uploaded files are stored under the thread workspace or artifact store with size/type limits, redaction policy, and model/tool visibility rules; attachment state survives restart and appears in diagnostics without exposing raw secrets. |
| Task 99 | Provider Profile Productization | Planned | Make model selection provider-aware instead of DeepSeek-only. | Settings, API `/models`, TUI picker, RoleModelResolver, ModelProviderFactory, and readiness checks support DeepSeek and OpenAI through the same profile schema; configured/requested/observed provider metadata is accurate; unsupported providers fail with actionable errors; Anthropic/Ollama remain future work until this provider contract is stable. |
| Task 100 | Repository Instruction Context | Planned | Make project instructions a deterministic context source for coding turns. | AGENTS.md, CLAUDE.md, README, and relevant project metadata are discovered once per turn through bounded repository tools or context middleware, injected with provenance and budget limits, and reflected in durable evidence; missing or conflicting instructions produce visible uncertainty instead of silent prompt assumptions. |
| Task 101 | TUI Evidence Preview | Planned | Let users inspect what the agent changed or produced without leaving the TUI. | Changed-file summaries can open bounded diff, file, and artifact previews; generated files are distinguishable from internal audit artifacts; previews are redacted, size-bounded, keyboard accessible, and recoverable after transcript reload. |
| Task 102 | TUI Execution Controls | Planned | Expose real execution mode and runtime health controls without turning the TUI into an operator dashboard. | Users can select solo/team execution for an advanced Run, see sandbox backend and health, see active run/route/worker state, and understand when a command is local trusted execution versus AIO Docker; ordinary input remains the primary route. |
| Task 103 | API Resource Completeness | Planned | Fill the API gaps that prevent clients from building a full product surface. | Thread memory, uploads, artifacts, usage, events, runs, messages, approvals, skills, MCP, tools, and diagnostics have consistent request validation, pagination or bounded limits, response schemas, structured errors, and OpenAPI descriptions; thread-level projections include ordinary turns and explicit Runs. |
| Task 104 | Onboarding And Human Error Recovery | Planned | Make first-run setup and common failures understandable to a new local user. | Welcome, `awesome init`, `/config`, `/model`, readiness, and provider-key errors explain what is missing, why it matters, where to set it on Windows/macOS/Linux, how to verify it, and what safe first command to try; Docker/AIO and LocalSandbox differences are explicit. |
| Task 105 | Headless Chat CLI | Planned | Add a non-TUI chat path for scripts, CI, and terminal users while reusing the same conversation authority. | `awesome-agent chat` can create/resume a thread, stream a durable conversation turn, pass model/memory/skill options, attach files when Task 98 is available, and render structured errors without duplicating AgentLoop or TUI logic. |
| Task 106 | Local API Hardening | Planned | Make API exposure rules match the product deployment boundary. | Loopback remains unauthenticated by default; non-loopback binding requires explicit auth/rate-limit configuration or is refused; readiness and docs identify the active security mode; rate limiting protects long-running run creation, uploads, and streaming endpoints. |
| Task 107 | Provider Ecosystem Expansion | Planned | Add provider breadth only after the provider-profile contract is no longer DeepSeek/OpenAI-specific. | Anthropic/Claude and local-model providers enter through the same ModelProvider, routing, streaming, usage, error, readiness, and profile contracts; provider additions include contract tests and product-surface metadata rather than one-off adapters. |
| Task 108 | Conversation Export | Planned | Add a bounded conversation export path after durable execution, memory, upload, and provider semantics are stable. | Users can export conversation history with thread metadata, model/usage summaries, changed-file summaries, attachment references, and redacted run evidence without exposing raw secrets, full tool output, or unbounded prompt context. |
| Task 109 | Extension Config UI | Planned | Add a local UI for inspecting and editing MCP and extension configuration after execution authority and extension parity are stable. | Users can inspect and update MCP sources, project skill roots, and extension health through a local UI that writes the same file-based project configuration, preserves redaction, and never bypasses extension capability policy. |
...
Tasks 92-109 are the committed product-closure sequence that must make local
