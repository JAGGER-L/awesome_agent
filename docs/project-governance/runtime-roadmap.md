...
exit conditions were broader than the actual code path: ordinary conversation
turns still bypass the durable Worker/Graph path, embedded `awesome` still has
...
- Local `awesome` runs ordinary turns directly in the launch/current working
...
- Host thread workspaces and git worktrees are not the default ordinary-turn
...
| --- | --- | --- | --- | --- |
| Task 76 | Embedded Local Runtime Host | Partial | Make `awesome` default to an embedded local runtime host where ordinary input can create resumable durable Runs without a running HTTP API. | Embedded ordinary turns now create durable conversation Runs and execute the local `conversation-turn` graph, but explicit `/run`, cancel, approval, budgets, artifacts, extension catalogs, and durable local runtime storage still need parity work. Task 95 owns that remaining closure. |
| Task 77 | Nonblocking Streaming And Resume | Partial | Make local and HTTP execution stream incrementally without freezing the TUI, and make interrupted ordinary turns resumable. | Nonblocking Textual workers and provider-stream rendering exist, but durable local resume is not wired and HTTP `resume_run_id` is not propagated through the conversation API. Task 94 owns resume/cancel/approval semantic closure. |
| Task 78 | Thread Session And Continuation UX | Done | Make thread navigation feel like conversation management instead of backend state dumping. | First ordinary input auto-creates a thread from launch context; `/new` and `/threads` switch/load conversations cleanly; duplicate titles are distinguishable; full UUIDs and container paths stay out of normal transcript output. |
...
| Task 83 | Leader AgentLoop Primary Turn Path | Done | Remove the ordinary-chat provider bypass and make every normal TUI/API conversation turn enter a durable Leader AgentLoop path. | Task 92 replaced the terminal `leader-turn` provider bypass with durable conversation Run intake, Worker routing, `ConversationGraph` execution, and runtime-event projection. |
...
| Task 91 | CLI Runtime Regression Fixes | Reopened | Correct the remaining product-path regressions where local `awesome` still behaved like direct provider chat for ordinary turns. | Ordinary local turns now use durable conversation Runs through Task 92. Remaining regressions are explicit `/run` planned-only behavior, local skills/MCP/cancel/approval placeholders, and guardrail/redaction gaps tracked by Tasks 93-98. |

Planned product closure sequence:

| Task | Phase | Status | Purpose | Exit condition |
| --- | --- | --- | --- | --- |
| Task 92 | Durable Conversation Turn Authority | Done | Make ordinary `/threads/{thread_id}/turns` a durable Leader Run/Worker/Graph path rooted in the user's trusted local current working directory, not a direct provider/tool loop. | Ordinary local/API turns create `conversation` Runs with Leader agents, `conversation-turn` routing, runtime events, user/assistant message writes owned by `ConversationGraph`, model/tool execution owned by the graph, usage and changed-file projection, and terminal Run state. `ConversationService` is now a runtime-event projection boundary. Checkpointing, full budget ledger parity, routed-provider streaming, and resume/cancel/approval remain in Tasks 95-97. |
...
| Task 95 | Real Embedded Local Runtime | Planned | Make embedded `awesome` use real local Run, Worker, Graph, trusted cwd, artifact, budget, extension, and conversation services. | Local ordinary turns already use real conversation Run/Graph semantics. This task removes the remaining planned/fake local runtime surfaces: explicit `/run`, cancel, approval, durable runtime repository storage, budgets, artifact evidence, worker-style lifecycle, and operational status parity. |
| Task 96 | Durable Turn Resume Cancel Approval | Planned | Close the lifecycle gap for interrupted or paused conversation turns across local and HTTP clients. | `resume_run_id` flows through TUI, surface clients, API schemas, and runtime services; `continue` / `resume` resumes the last resumable durable turn; cancellation interrupts active Worker/Graph work instead of only stopping rendering; approval decisions requeue the exact pending invocation; transcript and run state recover after process restart. |
| Task 97 | Runtime Streaming Event Projection | Planned | Add streaming over routed durable runtime execution without weakening checkpoint or Worker ownership. | Conversation graph emits basic text deltas, tool events, usage, messages, and terminal states, but routed provider streaming, richer reasoning/tool/team/validation progress, resumable SSE catch-up, and consistent HTTP/local event fanout are still incomplete. Non-streaming `complete()` remains valid only for internal operations where user-facing streaming has no value. |
| Task 98 | Local Capability Surface Parity | Planned | Wire local skills, MCP, community tools, and extension catalogs into the embedded trusted-cwd runtime with the same resolver/executor authority as API runs. | Local `list_skills`, `mcp_status`, `list_tools`, and staged skill selection read a real extension catalog; granted MCP/community tools execute through `ToolExecutor`, approval, timeouts, cancellation, durable invocation records, and `CapabilityResolver`; local tool execution inherits trusted cwd, guardrail, env-scrub, and redaction policy; denied tools are inspectable and cannot be exposed by prompt convention or bypassed by extension adapters. |
...
| Task 104 | TUI Execution Controls | Planned | Expose real execution mode and runtime health controls without turning the TUI into an operator dashboard. | Users can see trusted local cwd, sandbox backend, guardrail/env-scrub status, active run/route/worker state, and pending approvals; ordinary input remains the primary route and Docker/AIO controls are not required for the product closure path. |
| Task 105 | API Resource Completeness | Planned | Fill the API gaps that prevent clients from building a full product surface. | Thread memory, uploads, artifacts, usage, events, runs, messages, approvals, skills, MCP, tools, and diagnostics have consistent request validation, pagination or bounded limits, response schemas, structured errors, and OpenAPI descriptions; thread-level projections include ordinary turns and explicit Runs. |
