...
| Task 83 | Leader AgentLoop Primary Turn Path | Done | Remove the ordinary-chat provider bypass and make every normal TUI/API conversation turn enter a durable Leader AgentLoop path. | Ordinary input creates a durable turn/run with a leader agent; simple questions complete without tool calls; coding requests can use leader-visible tools; complex requests can create teammates/subagents/verifier; `ConversationService` no longer directly owns provider execution for normal turns. |
...
| Task 91 | CLI Runtime Regression Fixes | Done | Correct the remaining product-path regressions where local `awesome` still behaved like direct provider chat for ordinary turns. | Per-turn thinking reaches the provider; `thinking=off` suppresses reasoning UI; ordinary conversation turns can expose and execute leader-visible repository/sandbox tools against the launch workspace; embedded `/usage`, `/tools`, slash autocomplete, and local memory facts read real local runtime state; baseline tests cover the regression. |
