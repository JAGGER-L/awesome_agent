...
| --- | --- | --- | --- | --- |
| Task 76 | Embedded Local Runtime Host | Partial | Make `awesome` default to an embedded local runtime host where ordinary input can create resumable durable Runs without a running HTTP API. | `SurfaceClient` and `LocalRuntimeHost` exist, but the embedded host still uses an in-memory runtime repository and planned local Runs. Task 93 owns the remaining durable local runtime closure. |
...
| Task 83 | Leader AgentLoop Primary Turn Path | Reopened | Remove the ordinary-chat provider bypass and make every normal TUI/API conversation turn enter a durable Leader AgentLoop path. | Audit found `ConversationService.start_turn` still creates a terminal `leader-turn` Run and directly owns provider/tool execution for normal turns. Task 92 replaces this with the durable turn authority. |
...
| Task 91 | CLI Runtime Regression Fixes | Reopened | Correct the remaining product-path regressions where local `awesome` still behaved like direct provider chat for ordinary turns. | Per-turn thinking and local tool exposure improved, but ordinary turns still use the direct provider path, local `/run` is planned-only, and local skills/MCP/cancel/approval remain placeholders. Tasks 92-98 close the remaining runtime-product gap. |

Planned product closure sequence:

| Task | Phase | Status | Purpose | Exit condition |
| --- | --- | --- | --- | --- |
| Task 92 | Durable Conversation Turn Authority | Planned | Make ordinary `/threads/{thread_id}/turns` a durable Leader Run/Worker/Graph path rooted in the user's trusted local current working directory, not a direct provider/tool loop. | Every normal local/API turn creates or resumes a durable conversation Run with a Leader agent, graph thread id, checkpointed AgentLoop state, budget ledger, capability exposure, tool invocation records, runtime events, usage, cwd file/artifact projections, and structured failure state; `ConversationService` becomes a projection/orchestration boundary and no longer owns provider execution or writes conversation messages directly. |
...
| Task 94 | Output Redaction | Planned | Redact likely secrets before command output, logs, run evidence, or tool results enter model context or user-visible projections. | stdout/stderr, tool results, structured logs, diagnostics, and transcript projections redact common API keys, auth headers, private keys, JWTs, database URLs, and env assignments while preserving enough context for debugging; raw unredacted output is not persisted as durable run evidence by default. |
| Task 95 | Real Embedded Local Runtime | Planned | Make embedded `awesome` use real local Run, Worker, Graph, trusted cwd, artifact, budget, extension, and conversation services. | Local `awesome` has no planned/fake Run status, no fake cancel, no unsupported approval response, and no in-memory-only runtime authority; embedded and HTTP modes differ by transport/storage profile only, while `/run`, ordinary turns, cancel, approval, usage, tools, artifacts, guardrails, and errors share product semantics. |
...
| Task 97 | Runtime Streaming Event Projection | Planned | Add streaming over routed durable runtime execution without weakening checkpoint or Worker ownership. | Route-aware provider streaming is implemented or replaced by an equivalent event bridge; model token deltas, reasoning deltas, tool starts/results, team events, validation progress, usage updates, and terminal states stream from Worker/Graph events to conversation SSE and TUI; non-streaming `complete()` remains valid only for internal operations where user-facing streaming has no value. |
