...
| Task 97 | Runtime Streaming Event Projection | Planned | Add streaming over routed Worker-owned runtime execution without weakening checkpoint or dispatch ownership. | Conversation graph emits basic text deltas, tool events, usage, messages, and terminal states, but routed provider streaming, richer reasoning/tool/team/validation progress, resumable SSE catch-up, and consistent HTTP/local event fanout are still incomplete. HTTP and embedded local surfaces consume the same runtime-event projection produced by Worker-owned Runs; non-streaming `complete()` remains valid only for internal operations where user-facing streaming has no value. |
...
| Task 99 | Memory Lifecycle Integration | Planned | Connect builtin and provider memory to the actual AgentLoop lifecycle instead of only exposing settings and local facts. | Memory retrieval is injected through context middleware with source labels and budget controls; safe write candidates pass policy and are recorded to builtin/Mem0 when enabled; thread memory APIs list, delete, and disable facts; full source, raw conversations, secrets, and visible reasoning remain excluded. |
