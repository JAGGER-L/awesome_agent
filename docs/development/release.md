...
Awesome 1.2.0 is published as a manual GitHub Release. `VERSION` is the only
...
4. Inspect exactly four files under `dist/release`: `install.sh`, `install.ps1`,
   `awesome-1.2.0.zip`, and `SHA256SUMS`.
5. Verify the archive with:

   ```powershell
   uv run python scripts/release/verify_bundle.py `
     dist/release/awesome-1.2.0.zip 1.2.0
   ```

   This proves the bundled Core and TUI versions, fresh Schema 2 bootstrap,
   and read-only rejection of incompatible Application state. Awesome does not
   migrate or reinterpret databases from the discarded development schema.
...
2. From an up-to-date, clean `main`, rerun the Release Gate, rebuild the four
   ...
   feature branch.
3. Create and push tag `v1.2.0` at the verified `main` commit.
4. Create GitHub Release `v1.2.0` and upload the four files from
   `dist/release`.
