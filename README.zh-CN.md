...
Distributed Team Run 会在 root 范围内聚合 Leader、Teammate、Verifier 和 Subagent 的预算账本，并在图边界执行 token/active-time budget 检查。大型 team handoff、child result 和 verifier evidence payload 会卸载到 artifact，并通过 `context_compactions` 记录。Runtime 明确不做金额限制；内置资源控制以 token 和 active-time budget 为准。
