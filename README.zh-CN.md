...
`awesome-agent start` 是 fallback/debug supervisor，用于同时监督 API 和 Worker 进程。正常本地 API 开发请优先使用 `make dev`；如果需要由外部进程管理器分别管理它们，可以使用 `awesome-agent serve` 和 `awesome-agent worker`。本地 API 未内置认证，默认只绑定 loopback；绑定到非 loopback 地址需要显式 unsafe consent。
...
`awesome --api-url http://127.0.0.1:8000`。普通输入就是主要执行入口（Ordinary input is the main execution entry）；
`/run` 是高级/手动执行命令，不是让 agent 正常工作的默认方式。
