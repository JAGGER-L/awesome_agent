...
`awesome_agent` 是一个 local-first 的 Python coding agent runtime，目标是同时具备三类能力：
可在进程崩溃和审批等待中恢复的**持久化执行**、可追溯的**可观测性**，以及带独立
Verifier 的 Leader/Teammate/Subagent **多 Agent**组织。

本文档说明当前已经实现的能力，以及仍然属于原型或路线图的能力，避免夸大项目现状。
...
- repository-aware Run intake：CLI `--repo PATH` 会解析为已注册 repository UUID，经过
  allowed-root 策略、干净 base commit、每个 Run 独立命名 Git worktree，并使用可崩溃恢复的
  intake reservation；
- PostgreSQL dispatch queue 支持 `FOR UPDATE SKIP LOCKED` claim、lease、heartbeat、单调递增
  fencing token、delayed retry，以及 expired lease 到 `recovery_required` 的恢复；
- Worker 每次最多执行一个 Run，使用 `AsyncPostgresSaver` checkpoint，持有 lease heartbeat，
  并能在进程崩溃后从 checkpoint 恢复；
- provider-neutral structured model turn protocol 已映射到 DeepSeek Chat Completions 和
  OpenAI Responses；provider SDK 对象不会跨越 adapter 边界；
...
- **Read-only Coding Run** 通过带 checkpoint 的 `solo-readonly@1` Agent loop 执行，支持受限仓库
  ...
  证据门控的完成。
- **Modifying Coding Run** 路由到 `solo-modifying@1`，支持 `repo.apply_patch`、`repo.diff`、
...
`solo-modifying@1` 已接入持久化 approval interrupt/resume。模糊 shell 命令会创建 `approvals`
...
dashboard 和依赖感知 `/health` 仍属于后续工作。

## 技术栈

- Python 3.12 和 `uv`
- LangGraph，加项目自有 orchestration 和 provider interfaces
- 默认模型 provider 为 DeepSeek Chat Completions，也映射了 OpenAI Responses
...
可通过 `AWESOME_AGENT_LEADER_MODEL`、`AWESOME_AGENT_TEAMMATE_MODEL`、
`AWESOME_AGENT_VERIFIER_MODEL`、`AWESOME_AGENT_SUBAGENT_MODEL` 修改默认值，也可用
`AWESOME_AGENT_ROLE_MODEL_OVERRIDES` 的 JSON 覆盖具体 profile。未配置 DeepSeek API key 时，
Coding claim 会被禁用。
...
API 不接受任意 filesystem path。read-only 和 modifying Run 都要求原 checkout 干净，并基于捕获的
base commit 创建稳定 worktree。普通 `run` 命令会创建 modifying Coding Run；使用 `--read-only` 可禁用
修改工具。Modifying Run 只有在 required validation gates 通过后才会完成。
...
`awesome-agent start` 会监督相互独立的 API 和 Worker 子进程。需要分别管理进程时仍可使用 `serve` 和
`worker`。本地 FastAPI API 默认无认证且只绑定 `127.0.0.1`。绑定到非 loopback host 时必须显式传入
`--unsafe-bind-public`。

可通过 `GET /runs/{run_id}/dispatch` 查看 dispatch state。queued、retry-scheduled、waiting-approval、
...
打开 `http://127.0.0.1:4173`。页面包含 mock Agent topology、Todo、event trace、per-Agent
context、command approval、artifact 和移动端导航。这是 UI 原型，不是正在运行的多 Agent 系统。
